...
export async function POST(request: NextRequest) {
  ...
  if (!(file instanceof File)) {
    return NextResponse.json({ ok: false, message: 'Arquivo nao enviado.' }, { status: 400 });
  }

  if (!options.success) {
    return NextResponse.json({ ok: false, message: 'Opcoes invalidas para compressao de imagem.' }, { status: 400 });
  }

  if (!allowedTypes.has(file.type)) {
    return NextResponse.json({ ok: false, message: 'Tipo nao suportado. Use png/jpeg/webp/gif.' }, { status: 400 });
  }

  if (file.size > MAX_FILE_SIZE) {
    return NextResponse.json({ ok: false, message: 'Arquivo excede 10MB.' }, { status: 400 });
  }
  ...
  const outputBuffer =
    format === 'webp'
      ...
      : format === 'jpeg'
        ? await transformer.clone().jpeg({ quality: safeQuality, mozjpeg: true }).toBuffer()
        : format === 'png'
          ? await transformer.clone().png({ compressionLevel: 9, quality: safeQuality, palette: true }).toBuffer()
          : await transformer.clone().gif({ effort: 7, colours: 256 }).toBuffer();
