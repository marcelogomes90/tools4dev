      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <InputPanel>
          <p className="text-sm text-slate-700 dark:text-slate-300">
            Busca o IP visto pelo servidor (considerando headers de proxy como
        ...
        <OutputPanel>
          {result?.ok ? (
            <div className="space-y-2 text-sm">
              ...
              <p>
                <strong>x-forwarded-for:</strong> {result.forwardedFor ?? 'n/a'}
              </p>
              <p>
                <strong>x-real-ip:</strong> {result.realIp ?? 'n/a'}
              </p>
              <CopyButton value={result.ip ?? ''} label="Copiar IP" />
            </div>
          ) : (
            <p className="text-sm text-slate-700 dark:text-slate-300">
