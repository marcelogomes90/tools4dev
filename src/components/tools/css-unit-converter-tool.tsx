      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <InputPanel>
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div>
              ...
              <Select
                id="css-from"
                value={from}
                onChange={(event) => setFrom(event.target.value as CssUnit)}
              >
                {units.map((unit) => (
            ...
            <div>
              ...
              <Select
                id="css-to"
                value={to}
                onChange={(event) => setTo(event.target.value as CssUnit)}
              >
                {units.map((unit) => (
                  <option key={unit} value={unit}>
                    {unit}
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <Label htmlFor="css-root">Root font-size (px)</Label>
              <Input
                id="css-root"
                value={rootFontSize}
                onChange={(event) => setRootFontSize(event.target.value)}
              />
            </div>
            <div>
              <Label htmlFor="css-parent">Parent font-size (px)</Label>
              <Input
                id="css-parent"
                value={parentFontSize}
                onChange={(event) => setParentFontSize(event.target.value)}
              />
            </div>
            <div>
